...
public class TagViewGroup extends ViewGroup {
    ...
    public static final int TYPE_TAG_MORE = -1;
    public static final int TYPE_TAG_NORMAL = 1;
    ...
    private static final int DEFAULT_TEXT_BACKGROUND = R.drawable.list_item_bg;
    private static final int DEFAULT_VIEW_PADDING = 0;
    private static final int DEFAULT_TEXT_MARGIN_HORIZONTAL = 15;
    private static final int DEFAULT_TEXT_MARGIN_VERTICAL = 8;

    private static final int DEFAULT_TAG_RES_ID = R.layout.item_tag_view;
    ...
    public TagViewGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTagSize = a.getInteger(R.styleable.TagViewGroup_tvgTextSize, DEFAULT_TEXT_SIZE);
        mTagColor = a.getColor(R.styleable.TagViewGroup_tvgTextColor, DEFAULT_TEXT_COLOR);
        mBackground = a.getResourceId(R.styleable.TagViewGroup_tvgBackground, DEFAULT_TEXT_BACKGROUND);
        mViewBorder = a.getDimensionPixelSize(R.styleable.TagViewGroup_tvgBorder, DEFAULT_VIEW_PADDING);
        mTagBorderHor = a.getDimensionPixelSize(
                R.styleable.TagViewGroup_tvgItemBorderHorizontal, DEFAULT_TEXT_MARGIN_HORIZONTAL);
        mTagBorderVer = a.getDimensionPixelSize(
                R.styleable.TagViewGroup_tvgItemBorderVertical, DEFAULT_TEXT_MARGIN_VERTICAL);
        mCanTagClick = a.getBoolean(R.styleable.TagViewGroup_tvgCanTagClick, DEFAULT_CAN_TAG_CLICK);

        mRightImageResId = a.getResourceId(R.styleable.TagViewGroup_tvgRightResId, DEFAULT_RIGHT_IMAGE);
        mSingleLine = a.getBoolean(R.styleable.TagViewGroup_tvgSingleLine, DEFAULT_SINGLE_LINE);
        mShowRightImage = a.getBoolean(R.styleable.TagViewGroup_tvgShowRightImg, DEFAULT_SHOW_RIGHT_IMAGE);
        mShowEndText = a.getBoolean(R.styleable.TagViewGroup_tvgShowEndText, DEFAULT_SHOW_END_TEXT);
        endTextString = a.getString(R.styleable.TagViewGroup_tvgEndText);

        mTagResId = a.getResourceId(R.styleable.TagViewGroup_tvgTagResId, DEFAULT_TAG_RES_ID);
    ...
    private void initSingleLineView(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mShowEndText) {
            ...
            if (mTagResId == DEFAULT_TAG_RES_ID) {
    ...
    private int getSingleTotalHeight(int totalWidth, int totalHeight) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child.getTag() != null && child.getTag() == TYPE_TAG_NORMAL) {
    ...
    private int getTextTotalWidth() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            View child = getChildAt(i);
            if (child.getTag() != null && (int)child.getTag() == TYPE_TAG_NORMAL) {
    ...
    public void setTags(List<String> tagList) {
        ...
        if (tags != null && tags.size() > 0) {
            for (int i = 0, size = tags.size(); i < size; i++) {
                ...
                tagView.setTag(TYPE_TAG_NORMAL);
                final int finalI = i;
